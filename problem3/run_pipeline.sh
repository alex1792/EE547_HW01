...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    # 檢查容器是否還在運行
    if ! docker ps | grep -q pipeline-analyzer; then
        echo "Container stopped - pipeline complete!"
        break
    fi
    sleep 5
    ELAPSED=$((ELAPSED + 5))
...
# Extract results
mkdir -p output
# 從 volume 複製，而不是從停止的容器
docker run --rm -v pipeline-shared-data:/data -v $(pwd)/output:/output alpine sh -c "
    cp /data/analysis/final_report.json /output/
"
